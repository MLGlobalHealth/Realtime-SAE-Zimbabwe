...
df_popweight <- read_excel(here("data", "raw","mVAM", "WFP_mvam_weights_pop.xlsx")) 

df_raw_mVAM <- df_raw_mVAM |> mutate(
  hhh_edu = factor(hhh_edu,levels=c("preprimary/other", "primary", "secondary","higher"))
) 
df_popweight <- df_popweight |> select(ADM1_NAME_Survey,ADM2_NAME_Survey,Pop,
                                       HHHEduGrp_noSchool,HHHEduGrp_primary,
                                       HHHEduGrp_secondary, HHHEduGrp_higher) |>
  mutate(
    adm1_name = str_to_title(ADM1_NAME_Survey),
    adm2_name =str_to_title(ADM2_NAME_Survey),
  ) |>
  select(-ADM1_NAME_Survey,-ADM2_NAME_Survey)
colnames(df_popweight)[2:5] <- c("preprimary/other", "primary", "secondary","higher")   
...
df_smpweight_dmg <- df_raw_mVAM |> 
  ...
  summarise(n=n()) |> ungroup() |> left_join(
    (df_raw_mVAM |> group_by(obs_month,adm1_name) |> summarise(n_pm_padm1 = n())), 
    by = c('obs_month','adm1_name')
  ) |> 
  mutate(
    prop_sample = n/n_pm_padm1
  ) |> ungroup() |> select(-n, -n_pm_padm1)
...
vars_mod <- vars_mod[!vars_mod %in% c("hh_size_bucket", "h_toilet_type_3way", "h_toilet_type", "h_water_source_2way")]
...
# plot deciles
data_mics %>%
  mutate(deciles = cut(p_cell, breaks = quantile(p_cell, probs = seq(0, 1, 0.1)), include.lowest = T)) %>%
  group_by(deciles) %>%
...
write_csv(df_raw_mVAM %>% dplyr::select(respondent_id, p_cell), here("data", "processed", "cell_scores.csv"))
# ----------------------
...
data_zimvac <- data_zimvac |> 
  mutate(h_water_source = case_when(
    h_water_source_raw %in% c("Piped into dwelling",
                              "Piped into neighbour",
                              "Piped into public tap or standpipe",
                              " Piped into yard or plot"
    ) ~ "piped",
    h_water_source_raw%in% c("Other",
                             "Unprotected spring",
                             "Unprotected well",
                             "Sand abstraction",
...
vars_mod <- vars_mod[!vars_mod %in% c("fcs","obs_date","obs_week","obs_month", "obs_quarter", "obs_year",
                                      #"adm2_name",
                                      "hh_size_bucket", #hh_size
                                      "hhh_edu_raw",
                                      "h_toilet_type_3way", "h_toilet_type","h_toilet_type_raw",  #  h_toilet_type_2way ?
                                      "h_water_source_2way","h_water_source_raw" # use h_water_source
)
] 

df_cell <- data_zimvac[, c("h_cell", vars_mod)] |> mutate(
...
## Formula
formula <- cell ~ 1 +
  hh_size+hhh_edu+h_water_source+h_toilet_type_2way+hhh_sex +
  (1|adm2_name)
...
df_cell |>
  mutate(deciles = cut(p_cell, breaks = quantile(p_cell, probs = seq(0, 1, 0.1)), include.lowest = T)) %>%
  group_by(deciles) %>%
  dplyr::summarize(model = mean(p_cell,na.rm=T), actual = mean(cell,na.rm=T)) %>%
  pivot_longer(cols = c("model", "actual")) %>%
  ggplot(aes(x = deciles, y = value, color = name)) +
  geom_point() +
  theme_minimal()
