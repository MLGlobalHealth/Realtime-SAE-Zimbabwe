...
get_var_names <- function(mode) {
  ...
  if (mode == "mp_only") {
    variables.toinclude = variables.toinclude <- c(
      'p_cell',
...
#----------------------------
mode = "mp_only"
df_model <- readRDS(here(
  "data",
  "processed",
  paste0("df_model_", mode, ".rds")
))
variables.toinclude <- get_var_names(mode)
# read stanfit file
csv_files = read_rds(here(
  'output',
  'mcmc',
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mr_phat_', mode, '_', selected_model, '.csv')
  )
)
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mr_yhat_', mode, '_', selected_model, '.csv')
  )
)
...
df_mrp_dhs_ps <- MRP_post_samples(
  fit_rtm,
  df_mrp_dhs_newdata,
  month_idx_list,
  variables.toinclude,
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mrp_dhs_phat_', mode, '_', selected_model, '.csv')
  ),
  row.names = F
)
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mrp_dhs_yhat_', mode, '_', selected_model, '.csv')
  ),
  row.names = F
)
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mr_modalityMP_phat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
df_mr_ps_yhat <- df_mr_ps[[2]]
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mr_modalityMP_yhat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
# Change modality
...
for (var in variables.interaction) {
  int_var_name = paste0('modality_f2f:', var)
  df_mr_newdata[int_var_name] = df_mr_newdata['modality_f2f'] *
    df_mr_newdata[var]
}
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mr_modalityf2f_phat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
df_mr_ps_yhat <- df_mr_ps[[2]]
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mr_modalityf2f_yhat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
rm(df_mr_ps_phat, df_mr_ps_yhat, df_mr_ps)
...
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mrp_dhs_phat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
df_mrp_dhs_ps_yhat <- df_mrp_dhs_ps[[2]]
write.csv(
  ...
  here(
    'output',
    paste0('post_sample_mrp_dhs_yhat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
df.test <- read.csv(here(
  'output',
  paste0('post_sample_mrp_dhs_phat_joint_', selected_model, '.csv')
))
...
df_mrp_zimvac_ps <- MRP_post_samples(
  fit_joint,
  df_mrp_zimvac_newdata,
  month_idx_list,
  variables.toinclude,
  response = 'binary'
)
...
write.csv(
  df_mrp_zimvac_ps_phat,
  here(
    'output',
    paste0('post_sample_mrp_zimvac_phat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
df_mrp_zimvac_ps_yhat <- df_mrp_zimvac_ps[[2]]
write.csv(
  df_mrp_zimvac_ps_yhat,
  here(
    'output',
    paste0('post_sample_mrp_zimvac_yhat_joint_', selected_model, '.csv')
  ),
  row.names = F
)
