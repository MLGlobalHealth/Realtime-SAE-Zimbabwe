...
# - ZimVAC
zimvac23 <- read.csv(here("data", "processed", "zimvac2023_survey.csv")) |>
  select(adm1_name, adm2_name, obs_month, fcs)
zimvac24 <- read.csv(here("data", "processed", "zimvac2024_survey.csv")) |>
  select(adm1_name, adm2_name, obs_month, fcs)
zimvac <- bind_rows(zimvac23, zimvac24) |>
  mutate(poor = as.numeric(fcs < 28))
df_zimvac_est <- zimvac |>
...
rural_adm2 = df_zimvac_est$adm2_name[
  df_zimvac_est$obs_month ==
    as.POSIXct(c("2023-05-01"), format = "%Y-%m-%d", tz = "UTC")
] |>
  unique()
...
df_region_month_unique <- df_model |>
  select(adm1_name, adm1_index, adm2_index, adm2_name, obs_month, month_idx) |>
  unique()
df_weights <- readRDS(here("data", "processed", "mVAM_sample_weights.rds"))
df_weights <- df_weights |>
  filter(adm1_name %in% unique(df_model$adm1_name)) |>
  mutate(
...
df_direct_est <- df_direct_est_prep |>
  left_join(
    (df_direct_est_prep |>
...
df_zimvac_est$obs_month = as.POSIXct(
  df_zimvac_est$obs_month,
  format = "%Y-%m-%d",
  tz = "UTC"
)
...
df_mVAM_est_wald <- df_rtm_est |>
  ungroup() |>
  select(-starts_with("wald_"), -n_w, -y_est_se)
colnames(df_mVAM_est_wald) <- sub("wilson_", "", colnames(df_mVAM_est_wald))

df_zimvac_est_wald <- df_zimvac_est |>
  ungroup() |>
  select(-starts_with("zv_wald_"), -n, -zimvac_est_se)
colnames(df_zimvac_est_wald) <- sub("wilson_", "", colnames(df_zimvac_est_wald))

# -----------------------------------------------------------------------
# Modelled estimates and further processing
selected_model = 'model1'
df_region_unique <- df_model |>
  select(adm1_name, adm1_index, adm2_index, adm2_name) |>
  unique()
...
df_mr <- read.csv(here(
  'output',
  paste0('post_sample_mr_', type, 'hat_rtmonly_', selected_model, '.csv')
)) |>
  select(-X)
df_mr_est <- process_ps(df_mr, coverage_level)
df_mr_est <- df_mr_est |> mutate(type = 'MR', model = 'mobile-phone-only')
rm(df_mr)
df_mrp_dhs <- read.csv(here(
  'output',
  paste0('post_sample_mrp_dhs_', type, 'hat_rtmonly_', selected_model, '.csv')
))
df_mrp_dhs_est <- process_ps(df_mrp_dhs, coverage_level)
df_mrp_dhs_est <- df_mrp_dhs_est |>
  mutate(type = 'MRP (dhs)', model = 'mobile-phone-only')
rm(df_mrp_dhs)
...
df_mr_joint_est <- df_mr_joint_est |>
  mutate(type = 'MR-modalityPhone', model = 'joint')
rm(df_mr)
df_mr <- read.csv(here(
  'output',
  paste0(
    'post_sample_mr_modalityf2f_',
    type,
    'hat_joint_',
    selected_model,
    '.csv'
...
df_mr_joint_f2f_est <- df_mr_joint_f2f_est |>
  mutate(type = 'MR-modalityF2F', model = 'joint')
rm(df_mr)

df_mrp_dhs <- read.csv(here(
  'output',
  paste0('post_sample_mrp_dhs_', type, 'hat_joint_', selected_model, '.csv')
))
df_mrp_dhs_joint_est <- process_ps(df_mrp_dhs, coverage_level)
df_mrp_dhs_joint_est <- df_mrp_dhs_joint_est |>
  mutate(type = 'MRP (dhs)', model = 'joint')
rm(df_mrp_dhs)
df_mrp_zimvac <- read.csv(here(
  'output',
  paste0('post_sample_mrp_zimvac_', type, 'hat_joint_', selected_model, '.csv')
))
df_mrp_zimvac_joint_est <- process_ps(df_mrp_zimvac, coverage_level)
df_mrp_zimvac_joint_est <- df_mrp_zimvac_joint_est |>
  mutate(type = 'MRP (zimvac)', model = 'joint')
rm(df_mrp_zimvac)


####
months_of_interest <- as.POSIXct(
  c("2023-05-01", "2024-05-01"),
  format = "%Y-%m-%d",
  tz = "UTC"
)
####
df_combined <- bind_rows(
  ...
  df_mr_joint_est,
  df_mr_joint_f2f_est,
  df_mrp_dhs_joint_est,
  df_mrp_zimvac_joint_est
)

df_combined <- df_combined |>
  mutate(
    type_model = paste(type, model),
    obs_month = as.POSIXct(obs_month, format = "%Y-%m-%d", tz = "UTC")
  ) |>
  left_join(
    df_zimvac_est_wald |>
      mutate(
...
df.cis <- df_validation |>
  select(
    starts_with("ci_length"),
    starts_with("coverage"),
    starts_with("int_score"),
    type_model,
    type,
    model,
    obs_month
...
for (q in coverage_level) {
  df.cis[paste0('coverage_deviation', as.character(q * 100))] = df.cis[[paste0(
    'coverage',
    as.character(q * 100)
  )]] -
    q
...
df_validation_res <- df_validation |>
  ...
  ungroup() |>
  bind_cols(df.cis)
df_validation_res$type = factor(
  df_validation_res$type,
  levels = c(
    "Direct (mobile-phone)",
    "MR",
    "MR-modalityPhone",
...
df_validation_res$model = factor(
  df_validation_res$model,
  levels = c("", "mobile-phone-only", "joint")
)
...
d <- df_validation_res |>
  mutate(
    month = factor(
      obs_month,
      levels = c("2023-05-01", "2024-05-01"),
      labels = c("2023 May", "2024 May")
    ),
    method = factor(
      type_model,
      levels = c(
        "Direct (mobile-phone) ",
        "MR mobile-phone-only",
        "MRP (dhs) mobile-phone-only",
        "MR-modalityPhone joint",
        "MR-modalityF2F joint",
        "MRP (dhs) joint",
        "MRP (zimvac) joint"
      ),
      labels = c(
...
d |>
  select(month, model, type, pearson, spearman, ccc, rMSE, MAE, bias) |>
  knitr::kable()
d |>
  select(
    month,
...
d_long = d |>
  filter(
    type_model %in% c("mVAM direct", "MRP", "jMR-MP", "jMR-F2F", "jMRP")
  ) |>
  select(
    month,
    type_model,
...
df_plot_scatter <- df_combined |>
  mutate(
    month = case_when(
      obs_month == as.POSIXct('2023-05-01', tz = 'GMT') ~ "May 2023",
      T ~ "May 2024"
    )
  )
library(cowplot)
...
p = plot_grid(
  p1,
  p2,
  labels = c('May 2023', 'May 2024'),
  ncol = 1,
  label_size = 12
)
p
ggsave(
  here(paste0("output/corr-plot-validation.png")),
  p,
  width = 8,
  height = 10,
  bg = "white"
...
df_plot <- df_plot |>
  left_join(sf |> select(-adm1_name), by = 'adm2_name') |>
  st_as_sf() |>
  mutate(
    month = case_when(
      obs_month == as.POSIXct('2023-05-01', tz = 'GMT') ~ "May 2023",
...
pp1 = df_plot |>
  ...
  scale_fill_gradientn(
    values = c("1", "0.8", "0.6", "0.4", "0.3", "0.2", "0.1", "0.05", "0"),
    colors = c(
      "#9E3D21",
      "#BE4E21",
      "#DA6524",
  ...
  labs(
    fill = "Prevalence of households with \n poor food consumption patterns"
  ) +
...
ggsave(
  here(paste0("output/spatial-plots-validation.png")),
  pp,
  width = 8,
  height = 8.5,
  bg = "white"
...
df_plot_ci <- df_validation |>
  ...
  filter(
    obs_month %in% as.POSIXct(c('2023-05-01', "2024-05-01"), tz = "UTC")
  ) |>
  bind_rows(
    df_zimvac_est_wald |>
      filter(
...
pp1 = df_plot_ci |>
  mutate(ci_length90 = ci_length90 * 100) |>
  filter(obs_month == as.POSIXct('2023-05-01', tz = 'GMT')) |>
...
ggsave(
  here(paste0("output/spatial-plots-ci90length.png")),
  pp,
  width = 8,
  height = 8.5,
  bg = "white"
