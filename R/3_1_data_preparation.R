...
p_cell_zimvac <- read_csv(here(
  "data",
  "processed",
  "cell_scores_ZIMVAC.csv"
)) |>
  distinct()

# read zimvac(f2f) data
df_raw_zimvac23 <- read_csv(here(
  "data",
  "raw",
  "ZimVAC",
  "zimvac2023_survey.csv"
))
df_raw_zimvac24 <- read_csv(here(
  "data",
  "raw",
  "ZimVAC",
...
df_raw_mVAM_mp <- df_raw_mVAM |>
  left_join(p_cell, by = "respondent_id") |>
  mutate(model = 'mobile-phone')
df_raw_mVAM_joint <- df_raw_mVAM |>
  left_join(p_cell_zimvac, by = "respondent_id") |>
  mutate(model = 'joint')
...
df_zimvac23 <- df_raw_zimvac23[, c(
  subset(
    colnames(df_raw_zimvac23),
    colnames(df_raw_zimvac23) %in% colnames(df_mVAM)
  ),
  "fcs_raw"
)] |>
  mutate(
...
df_zimvac24 <- df_raw_zimvac24[, c(
  subset(
    colnames(df_raw_zimvac24),
    colnames(df_raw_zimvac24) %in% colnames(df_mVAM)
  ),
  "fcs_raw"
)] |>
  mutate(
...
popframe_zimvac23ind <- read_csv(here(
  "data",
  "processed",
  "popframe_zimvac23ind_freq.csv"
))
...
processing_for_mlr <- function(df, type = "observation") {
  ...
  df <- df |>
    mutate(
      ...
      h_water_source_2way = factor(
        h_water_source_2way,
        levels = c("other", "improved")
      ),
      h_toilet_type_2way = factor(
        h_toilet_type_2way,
        levels = c("unimproved", "improved")
      ),
      # For multiclass categorical
      hh_size_bucket = factor(
        hh_size_bucket,
  ...
  df_region_index <- as.data.frame((sf |> select(adm1_name, adm2_name))) |>
    select(-geometry) |>
    mutate(
      adm2_index = 1:nrow(sf),
      adm1_index = as.factor(adm1_name) |> as.integer()
    ) |>
    select(-adm1_name)
...
df <- processing_for_mlr(
  bind_rows(df_mVAM, df_zimvac23 |> mutate(model = 'joint')),
  type = "observation"
)
