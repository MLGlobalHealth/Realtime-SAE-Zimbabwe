...
pop_frame_0 <- dhs %>%
  ...
  summarize(
    pop_weight = sum(hh_weight) / 100000,
...
for (ii in 1:nrow(missing_strata)) {
  ...
  pop_frame_1 <- bind_rows(
    pop_frame_1,
    pop_frame_0 %>%
      filter(
        get(missing_strata[ii, 'var']) != missing_strata[ii, 'level'],
        adm2_name == missing_strata[ii, 'adm2_name']
      ) %>%
      mutate(
        (!!missing_strata[ii, 'var']) := missing_strata[ii, 'level'],
        pop_weight = 0.01
      )
  )
}
...
popframe <- weightFrame(raw = pop_frame_1, weighting_vars = wv)
...
write_csv(
  popframe$weighted,
  file = here(paste0('data/processed/popframe_dhs_census22.csv'))
)
...
write_csv(
  popframe_zimvac23,
  file = here('data/processed/popframe_zimvac23ind_freq.csv')
)
popframe_zimvac23 <- popframe_zimvac23 |> select(-n, n_adm2, prop_cell_byadm2)
write_csv(
  popframe_zimvac23,
  file = here('data/processed/popframe_zimvac23ind.csv')
)
