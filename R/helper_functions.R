...
prepare_bym2 <- function(adj_mat) {
  ...
  inla_adj <- sparseMatrix(i = nodes$n1, j = nodes$n2, x = 1, symmetric = T)
  ...
  Q_inv = inla.qinv(
    Q_jit,
    constr = list(A = matrix(1, 1, nrow(adj_mat)), e = 0)
  )
...
post_pred_each <- function(
  ...
  df_res = df |>
    ...
    summarise(prev = weighted.mean(phat, w), .groups = "rowwise") |>
    ungroup()
...
post_pred_sample <- function(
  ...
  for (j in 1:N_draw) {
    ...
    df_tmp <- post_pred_each(
      ...
      psi = matrix(
        unlist(df_psi[j, ]),
        nrow = length(unique(df_model$month_idx)),
...
MR_post_samples <- function(
  ...
  vec_b0 = fit$draws(format = "df", variables = c('b0'), inc_warmup = F) |>
    ...
    unlist() |>
    unname()
  df_betas = fit$draws(format = "df", variables = c('betas'), inc_warmup = F) |>
    select(-.chain, -.iteration, -.draw)
  df_phi = fit$draws(format = "df", variables = c('phi'), inc_warmup = F) |>
    select(-.chain, -.iteration, -.draw)
  df_theta = fit$draws(format = "df", variables = c('theta'), inc_warmup = F) |>
    select(-.chain, -.iteration, -.draw)
  df_psi = fit$draws(format = "df", variables = c('psi'), inc_warmup = F) |>
    select(-.chain, -.iteration, -.draw)
  df_nu = fit$draws(format = "df", variables = c('nu'), inc_warmup = F) |>
    select(-.chain, -.iteration, -.draw)
  df_xi = fit$draws(format = "df", variables = c('xi'), inc_warmup = F) |>
    select(-.chain, -.iteration, -.draw)

  df_ps = post_pred_sample(
    vec_b0,
    df_betas,
    df_phi,
    df_theta,
...
dummy_guide <- function(
  ...
  dummy_geom <- geom_point(
    data = data.frame(x = rep(Inf, n), y = rep(Inf, n), dummy = factor(labels)),
    aes(x, y, dummy = dummy),
    alpha = 0,
    key_glyph = key
  )
  ...
  if (phat == 0) {
    ...
    return(
      (1 / (1 + (1 / n) * (z^2))) *
        (phat +
          (z^2) / (2 * n) -
          ((z) / (2 * n)) * sqrt(4 * n * phat * (1 - phat) + z^2))
    )
  }
}
...
compute_wilson <- function(est, n, z) {
  if (z < 0) wilson_lower(phat = est, n = n, z = -z) else
    wilson_upper(phat = est, n = n, z = z)
}
...
process_ps <- function(df_ps, coverage_level = c(0.8, 0.9)) {
  df_ps <- df_ps |>
    mutate(month_idx = factor(time_index)) |>
    select(-time_index)
  quantiles = c()
  q_names = c()
  ...
  df_est <- bind_cols(
    ...
    (df_ps |>
      select(-adm2_index, -month_idx) |>
      pmap_df(~ as.list(quantile(c(...), quantiles))) |>
      bind_cols())
  )
...
process_direct <- function(df, coverage_level = c(0.8, 0.9)) {
  ...
  for (q in coverage_level) {
    ...
    q_names_wald = c(
      q_names_wald,
      paste0(c('wald_low', 'wald_high'), as.character(q * 100))
    )
    q_names_wilson = c(
      q_names_wilson,
      paste0(c('wilson_low', 'wilson_high'), as.character(q * 100))
    )
  }
  ...
  wilson_cis <- lapply(z_scores, compute_wilson, est = df$y_est, n = df$n_w) |>
    bind_cols()
...
process_zimvac <- function(df, coverage_level = c(0.8, 0.9)) {
  ...
  for (q in coverage_level) {
    ...
    q_names_wald = c(
      q_names_wald,
      paste0(c('zv_wald_low', 'zv_wald_high'), as.character(q * 100))
    )
    q_names_wilson = c(
      q_names_wilson,
      paste0(c('zv_wilson_low', 'zv_wilson_high'), as.character(q * 100))
    )
  }
