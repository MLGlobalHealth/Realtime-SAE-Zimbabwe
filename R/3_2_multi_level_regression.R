...
##---  Choose model
params <- c('b0','betas','sigma_phi','sigma_theta','sigma_nu','sigma_xi','sigma_psi')
params_all <- c('b0',
                paste0('betas',as.character(1:length(variables.toinclude))),
                'sigma_phi','sigma_theta','sigma_nu','sigma_xi','sigma_psi')
...
fit$save_output_files(dir = here('outputs','mcmc'), basename = paste0("stan_mcmc_",mode,"_",selected_model), timestamp = FALSE)
csv_files <- fit$output_files()
write_rds(csv_files, here('output','mcmc',paste0('cmdstanr_csv_file_names_',mode,"_",selected_model,'.rds')))
...
colnames(post_samples) <- colnames(post_samples) %>%
  ...
  gsub(",","_", .)
# hyper-parameters
for (param in c('sigma_phi','sigma_theta','sigma_nu','sigma_xi','sigma_psi')){
  dat = post_samples
...
## summary
df_result  = fit$draws(format = "df", variables=params,  inc_warmup = F) |> select(-.chain,-.iteration,-.draw)
res_table = cbind(colMeans(df_result),t(apply(df_result, 2, quantile, c(0.025,0.975))))
